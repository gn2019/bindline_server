...
async function sample1() {
    await setExistingScore('Foxa3');
    setSequences({'WT': 'ATCGATCGATCG', 'ins': 'ATCGATAAACGATCG'});
    send();
}


function sample2() {
    setSequences({'WT': 'AGGGCGGGATTGGCGTTTTA'});
...
async function sample3() {
    await setExistingScore('Six4');
    setSequences({'WT': 'ATCGATCGATCG'});
    setSearchSignificatMutations();
    setShowDiffOnly();
    setScoreThreshold('zscore', 2.5);
    setTabbedView();
    send();
}
